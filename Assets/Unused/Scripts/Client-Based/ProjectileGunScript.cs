...
public class ProjectileGunScript : MonoBehaviour
{
    ...
    public Vector3 appliedTorque = new();
    float tempDamage;
    ...
    private void Shoot() {
        ...
        for (int x = 0; x < bulletsShot; x++) {
            GameObject projectile = PhotonNetwork.Instantiate(projectileObject, bulletSpawn.position, bulletSpawn.rotation);
            ...
            projectile.GetComponent<Rigidbody>().AddTorque(appliedTorque);
        }
